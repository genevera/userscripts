...
// @include     https://*
// @updateURL   https://github.com/bewam/userscripts/raw/master/stlify/https_links.user.js
// @version     1.1
...
if(info.includes.length < 2) {
    if(info.includes[0] == "https://*")
        throw(info.localizedName +
            ': you need to modify the @include parameter');
}
...
// build selector
var selector = myAttrs.map((e) => '[' + e + '^="http:"]')
    .join(',');
// add links without protocol defined: "//"
selector += ',';
selector += myAttrs.map((e) => '[' + e + '^="//"]')
    .join(',');
...
function secureDomainLink(el) {
    ...
    function replaceAttr(attrName) {
        el.setAttribute(attrName, el.getAttribute(attrName)
            .replace(
                /^http:/, 'https:'));
    }
    for(var myAttr of myAttrs) {
        if(
            ...
            .indexOf(location.hostname) > -1 // don't rewrite external links
        ) {
            replaceAttr(myAttr);
        }
    }
